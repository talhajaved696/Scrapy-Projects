# -*- coding: utf-8 -*-
import scrapy
from scrapy_selenium import SeleniumRequest

class ComputerdealsSpider(scrapy.Spider):
    name = 'computerdeals'

    def start_requests(self):
        yield SeleniumRequest(url="https://slickdeals.net/computer-deals/",wait_time=3,callback=self.parse)

    def parse(self, response):
        products = response.xpath("//ul[@class='dealTiles categoryGridDeals']/li")
        for product in products:
            try:
                store1 = product.xpath(".//button[@class='itemStore bp-p-storeLink bp-c-linkableButton bp-c-button--link bp-c-button']/text()").get()
            except:
                store1 = product.xpath(".//a[@class='itemStore bp-p-storeLink bp-c-link']/text()").get()
            yield {
                'Title':product.xpath(".//a[@class='itemTitle bp-c-link']/text()").get(),
                'Link':product.xpath(".//a[@class='itemTitle bp-c-link']/@href").get(),
                'Store': store1,
                'Price':product.xpath("normalize-space(.//div[@class='itemPrice  wide ']/text())").get()
            }

            next_page = response.xpath("//a[@data-role='next-page']/@href").get()
            if next_page:
                abs = f'https://slickdeals.net{next_page}'
                yield SeleniumRequest(url=abs,wait_time=3,callback=self.parse)